...
const createAccounts = async (number: number): Promise<void> => {
    ...
    let FINALISATION_TIMEOUT = 30000;

    for (let seed = 1000; seed < 1000 + number; seed++) {
...
const spam = async (argv: spamOptions) => {
    ...
    for (let i = 0; i < txCount; i++) {
        ...
        let nonce = nonces[i];
        nonces[i]++;
        let senderKeyPair = keyring.addFromUri(seedFromNum(k));
    ...
    for (let i = 0; i < txCount; i++) {
        try {
            await transactions[i].send(({ status }) => {
...
async function getBlockStats(hash?: BlockHash | undefined): Promise<any> {
    ...
    return {
        ...
        blockNumber: signedBlock.block.header.number.toHuman(),
    }
}
